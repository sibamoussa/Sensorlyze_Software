#This script includes the class responsible for analyte quantification
import pandas as pd
...
############ TAB2 #########
            ...
            done_selection_button = QPushButton("Done Point Selection", self.window)
            ...
            input_timerange_widget = QLabel("Please input the sampling range over which the values are to be averaged (seconds): ", self.window)
            ...
            def data_selection(self):
                ...
                def update_region(self):
                    ...
                    region = np.round(region, decimals=0)
                    df2= np.array(df)
...
############# TAB3 #########
    ...
    def apply_filter_quant(self, state):
        if noise_filter_button.isChecked():
            ...
            pen = pg.mkPen(color=(100, 200, 100),width=3)
            ...
            graphWidget2.show()
            self.window.tab2.layout.addWidget(graphWidget2,3,0,20,5)
    ...
    def export_points_quant(self):
        ...
        current_sel_.columns = pd.MultiIndex.from_product([['Mean Currents (A)'], current_sel_.columns])

        writer = pd.ExcelWriter('Quant_Selected_Points.xlsx')
