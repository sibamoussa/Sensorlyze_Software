...
##This class is responsible for the single sensor calibration curves
class single_cal():
...
############ TAB2 #########
            ...
            plot_normalizedcal_button = QPushButton('Plot Normalized Cal', self.window)
            ...
            clear_plot_button = QPushButton('Clear All', self.window)
...
##this is where i stopped
    def plot_file_single_tab2(self):
        ...
        graphWidget.plot(df[:,0], df[:,1],pen=pen)
        styles = {'color': 'k', 'font-size': '30px', 'font-weight': 'bold', 'font-type': 'arial'}
        graphWidget.setLabel('left', 'Current / pA', **styles)
        graphWidget.setLabel('bottom', 'Time / s', **styles)
        graphWidget.setBackground('w')
        # check this
        headers = ["Time (s)", "Current (A)"]
        global currents, df_raw
        df_raw =pd.DataFrame(df) #, index=headers)
        currents = []
        graphWidget.show()
    ...
    def plot_cal_single(self):
        ...
        if (check_same_size):
            ...
            global calib_data
            calib_data = pd.DataFrame({'Concentration (uM)': concs_values, 'Current (pA)': currents})  # df_new2[2]]
    ...
    def export_normal_plt_single(self):
        if regress_data_button.isChecked():
            norm_calib_summary_data = pd.DataFrame(norm_calib_summary, index=['R_squared', 'Intercept', 'Slope'], columns=[''])  # ,columns=['1','2','3'])  # df_new2[2]]
            # create excel writer object
            writer = pd.ExcelWriter('Results.xlsx')
            # write dataframe to excel
