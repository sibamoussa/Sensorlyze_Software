...
############ TAB1 ########
            ...
            filetype_list = ["", "HEKA (.asc)", "Biologic (.txt)","CH (.txt)","Sensolytics (.dat)","Excel (.xlsx)"]
            ...
            def filelist_selections_tab2(self):
                ...
                if filetype_selection == "HEKA (.asc)":
                    ...
                    if extension == '.txt':
                        ...
                        msg.setWindowTitle("Error")
                        returnValue = msg.exec()
                elif filetype_selection == 'Sensolytics (.dat)':
                    if extension == '.dat':
                        with open(fileName,'r') as expfile:
                            data = []
                            header = []
                            index = 0
                            #Read sensolytics data line by line because complicated data
                            for line in expfile:
                                if line[0] == '#':
                                    line = line.split('\t')
                        ...
                        # checking number of channels here, note: channels must be between 2 and 4
                        if any('2' in string for string in header[2]):
                            df.drop(df.columns[[2]], axis=1, inplace=True)

                            if any('uA' in string for string in header[6]):
                                df.iloc[:,1]= df.iloc[:,1] * 10 ** -3
                            elif any('mA' in string for string in header[6]):
                                df.iloc[:,1] = df.iloc[:,1] * 10 ** -6
                            elif any('nA' in string for string in header[6]):
                                df.iloc[:,1] = df.iloc[:,1] * 10 ** -9
                                print(df)
                            else:
                                    pass
                            df.columns = ["Time (s)", "Current (A)"]

                        elif any('3' in string for string in header[2]):
                            df.drop(df.columns[[3]], axis=1, inplace=True)
                            #Pulsed amperometry
                            if any('Pul' in string for string in header[1]):
                                if any('uA' in string for string in header[8]):
                                    df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -3
                                elif any('mA' in string for string in header[8]):
                                    df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -6
                                elif any('nA' in string for string in header[8]):
                                    df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -9
                                else:
                                    pass
                                df.columns = ["Time (s)","Potential (V)","Current (A)"]
                                #rearranging column order
                                df = df[['Time (s)', 'Current (A)', 'Potential (V)']]
                                ...
                                if any('uA' in string for string in header[8]):
                                    ...
                                    pass
                                df.columns = ["Time (s)", "Current WE1 (A)", "Current WE2 (A)"]

                        elif any('4' in string for string in header[2]):
                            # 2 WE amperometry
                            df.drop(df.columns[[4]], axis=1, inplace=True)

                            if any('uA' in string for string in header[8]):
                                df.iloc[:, 3] = df.iloc[:, 3] * 10 ** -3
                                df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -3
                            elif any('mA' in string for string in header[8]):
                                df.iloc[:, 3] = df.iloc[:, 3] * 10 ** -6
                                df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -6
                            elif any('nA' in string for string in header[8]):
                                df.iloc[:, 3] = df.iloc[:, 3] * 10 ** -9
                                df.iloc[:, 2] = df.iloc[:, 2] * 10 ** -9
                            else:
                                pass

                            df.columns = ["Time (s)", "Potential (V)","Current WE1 (A)", "Current WE2 (A)"]
                            df = df[["Time (s)", "Current WE1 (A)", "Current WE2 (A)","Potential (V)",]]

                        else:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Critical)
                            msg.setText("Sensorlyze can only process Sensorlytics files up to 4 channels with two working electrodes.")
                            msg.setWindowTitle("Error")
                            returnValue = msg.exec()
